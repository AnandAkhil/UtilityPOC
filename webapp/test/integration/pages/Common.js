sap.ui.define([
	...
	function getFrameUrl (sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("energymeterreading/test/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#ABCPowerCorp-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#ABCPowerCorp-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("energymeterreading.test.test.integration.pages.Common", {
		...
		getMockServer : function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["energymeterreading/test/localService/mockserver"], function (mockserver) {
